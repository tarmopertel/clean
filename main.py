...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized service description that only this customer will receive; 
    ...
    CUSTOMER place of Residence: {residence};
    ...
    PRODUCT DESCRIPTION: describe the service in 5 sentences;
    BENEFITS: describe in 3 sentences why this service is perfect considering customers age group and place of residence;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account place of residence {residence} and age {agegroup}, write a story in first person, example " I started my morning in the...";
"""

prompt = PromptTemplate(
    input_variables=["agegroup", " residence", "content"],
    template=template,
)
...
with col1:
    st.markdown("Otstarve: teenusetutvustustekstide personaliseerimine igale kliendile või kliendigrupile; väljundtekst on kohandatud kliendi a) vanuserühmaga ja b) elukohaga; sisendtekstiks on neutraalses vormis teenusekirjeldus. \
    \n\n Kasutusjuhend: 1) valmista ette teenuseekirjeldus (sisendtekst). 2) määra tarbijasegemendid lähtuvalt vanuserühma ja elukohtade kombinatsioonidest. 3) sisesta ükshaaval tarbijasegmentide lõikes eeltoodud info äpi kasutajaliideses, saada ära. \
    4) kopeeri ükshaaval tarbijasegmentide lõikes äpi väljundteksti kõnealuse teenuse tutvustuslehele.")

with col2:
    st.image(image='companylogo.jpg', caption='Probably best cleaning servicein the Estonia)
